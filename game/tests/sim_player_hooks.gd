...
func test_hook_data_is_sound() -> void:
	...
	assert_eq(hooks, 24)  # M10.1: + the search for Mrsha, the rope at the rift; M9.4: + the bee raid, the painted Soldiers, Zel, Frozen, Erin's leaving; M9.1: + player_stood_with_ryoka, player_ate_at_lyonettes_inn; M9.2: + player_sat_with_ryoka_and_fals; M9.3: + player_watched_erins_play, player_held_the_esthelm_barricade
