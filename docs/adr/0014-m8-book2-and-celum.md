...
## M8.7 Canon Interlude – Quiet Discussions + 2.39–2.48 (days 67–71)

User choices (2026-09-26): Erin moves to Celum from day 71 and lives at the Frenzied Hare; the Hare gets its own indoor map; four stages (Snow Golems, the sledding crowd, the Celum muggers, the Hare bar fight); Toren leaves the map from day 70 (his road killings are news only).

Built in three steps. World data by hand; canon JSON by a delegated agent (chapters read outside the main session, per `CLAUDE.md` context rules); then re-verified in the main session (validator, full GUT, Python, a new stage test, a screenshot).

**Threads.** Ryoka returns with Mrsha, fights and makes up with Erin, tells Liscor the Stone Spears are gone and takes Erin's debt to the Silverfangs on herself; Rags is sent away and banned from the inn. Klbkch shares Antinium secrets and backs Ryoka's level-cap idea (total levels, not age), kept secret with Selys. Erin builds a sledge that Toren pulls; Snow Golems attack; Toren pulls a sledding crowd by Liscor. On day 70 Toren drags the sleeping Erin far north and leaves her. On day 71 Erin fights wolves and a bear, is guided to Celum by Jerad Riels, beats three muggers, meets Octavia, and takes over the kitchen of Agnes's run-down Frenzied Hare, where the drunk Brilliant Swords start a brawl. Ryoka runs to Teriarch, sees his true form, loses a riddle game, and has him scry Erin; he gives her a Rihal Imperium tome for the Gnolls, copies her iPhone and sends her to Stitchworks. Book 2 ends with Esthelm burned by Rags' tribe, the Liscor dungeon dug open (Griffon Hunt enters), [Dangersense] firing across the continent, Zel silenced at a Drake assembly, and Az'kerash sending Venitra after Ryoka.

**Timeline (guesses).** 67: Quiet Discussions, 2.39. 68: 2.40. 69: 2.41, 2.42, the 2.43 night. 70: Toren drags Erin north; the search. 71: Ryoka's run through 2.48. The text does not agree with itself on Erin's timing (2.43/2.44 hint she reaches Celum on day 70, but Ryoka leaves the next morning and meets her in Celum the same day as 2.47); day 71 was chosen and each note says why. Remote threads (Niers, BlackMage, Esthelm, Venitra, Magnolia, the Drake assembly) use placeholder days.

**World data.** New indoor map `celum_frenzied_hare` (location `frenzied_hare`): a door from `celum_square` (14,3), a bar (shop `frenzied_hare`: hot meal, bread), a stove, two tables, a rented bed (6c). Stage-only enemies `snow_golem`, `celum_mugger`, `brilliant_swords_adventurer`, helper `frenzied_hare_regular`. `npc_behaviour`: Erin's inn goals stop on `erin.left_liscor`; she is off the map on `erin.stranded_north` and works, sleeps and visits Stitchworks in Celum on `erin.in_celum`; Toren leaves on `toren.left_erin`; Rags stops visiting on `rags.banned_from_inn`. The hunger `fed_line` no longer names Erin (Lyonette runs the inn kitchen now). No engine change, no save change (v11).

**Canon data.** 45 events in 11 chapter files. 12 new NPCs (umina, cameral, marian, venaz, blackmage, jerad_riels, grev, agnes, maran, safry, venitra, serpentine_matriarch_of_zeres); 6 new locations (baleros, liscor_park, giant_bee_cave, north_of_the_high_passes, zeres, drake_assembly_hall - a guess, the hall is unnamed) plus `frenzied_hare`. Tier 1 news: the Stone Spears' fall and Zel's defeat, Esthelm burned, the Liscor dungeon open, the Drake assembly silencing Zel. Niers's entry corrected: a Fraerling, Level 64 [Grandmaster Strategist] (the M8.2 entry had Human, Level 63).

**Stages.** `b2.snow_golems_attack_erins_sledge` (floodplains_south, 10–14), `b2.toren_pulls_a_sledding_crowd` (scene, inn_hill, 14–18; Toren, Selys, Klbkch, Krshia), `b2.erin_beats_the_celum_muggers` (celum_square, 14–18; Wesle in the second wave), `b2.bar_fight_with_the_brilliant_swords` (celum_frenzied_hare, 19–23; regulars as helpers - a design choice, most guests flee in the text). Ryoka and Garia have no `npc_behaviour` entry, so they keep story roles only. No new hooks.

**History, not events.** The two Antinium Wars interludes are an in-world history book Ryoka reads. No events or chapter files. Facts kept for later books: Klbkch is a Prognugator who survived the First Antinium War; Zel won the name Tidebreaker holding Geir's dungeon; Magnolia's army broke the siege of Liscor; General Sserys died with the First Unified Army.

**Flagged, not linked.** Selys "Shivertail" vs Zel Shivertail; the Goblin Lord's eyes vs Az'kerash; Mihaela vs Valceif Godfrey; Lyonette's [Princess] reveal vs Book 1's `princess_thief`; "Terrandria" spelling in 2.42. Perril Chandler = Az'kerash is stated in 2.48. 2.35 (M8.4) set `ryoka.gained_first_class`, but the text has a faerie cancel the level-ups and 2.39 has Ryoka with no levels. User (2026-09-26): Ryoka never gains a level. Fixed after M8.7: 2.35 now sets `ryoka.class_offer_cancelled` (the event id is kept for old saves), and the [Barefoot Runner] system records in 1.20R and 2.35 became `other` records of an offered, refused class. The Book 2 epub has web-serial author's notes (the Mating Rituals interlude, 2.48); they are ignored.

**Tests.** `sim_canon_book2`: `LAST_DAY` 67 → 71, drift 0; end state (Erin at the Hare, Toren off the map, Esthelm burned, the dungeon open). New `sim_erin_in_celum` (Erin, Toren and Rags follow the flags; the Hare's door, bar and room; save/load) and `sim_book2_end_stages` (all four stages start on the real data). GUT 591/591 (63 scripts); validator 0 errors (`--all`); Python 51/51. Checked on screen (the bar fight on day 71).
